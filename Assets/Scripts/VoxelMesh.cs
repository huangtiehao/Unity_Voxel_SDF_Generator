...
namespace VoxelSystem
{
    ...
    public class VoxelMesh {
		...
		public static Mesh Build(Voxel[] voxels, float unit, bool useUV = false) {
		...
		static void CalculatePlane (
			...
			Voxel voxel, bool useUV, Vector3 offset, Vector3 right, Vector3 up, Vector3 normal, int rSegments = 2, int uSegments = 2
		) {
			float rInv = 1f / (rSegments - 1);
			float uInv = 1f / (uSegments - 1);

			int triangleOffset = vertices.Count;
            var center = voxel.worldPos;
